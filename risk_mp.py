...
def get_result(num_armies, attack_dice, defend_dice, que, i, num_simulations, num_procs):
    for j in range(i+1, num_simulations+1, num_procs):
...
def run_simulations(num_simulations, num_armies, attack_dice, defend_dice, que):
    ...
    for i in range(0, num_procs):
        p = mp.Process(target=get_result, args=[num_armies, attack_dice, defend_dice, que, i, num_simulations, num_procs])
        processes.append(p)
        p.start()
